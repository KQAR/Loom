...
| **PrivilegedHelperClient** | client | two halves, only one live: the shipping `setSystemProxy` / `systemProxySnapshots` path (direct `networksetup` + osascript, no helper) **and** the SMAppService register/approve + XPC surface, which is **dormant by decision** (ad-hoc signing — ROADMAP § M2) |
...
| **LoomHelper** | tool | root daemon: per-service proxy backup/override/restore, CA trust install/verify, caller + Apple-binary validation, crash watchdog, idle-exit — **dormant by decision**, never loaded under ad-hoc signing |
...
- **NIO modules are Swift 5.** Do not flip `ProxyCore`/`MCPServer` to Swift 6 without reworking the channel handlers off `@unchecked Sendable`. `PrivilegedHelperClient` and `LoomHelper` are also Swift 5 (XPC + continuations / shared mutable state + locks).
...
- **HTTPS interception works (M2); one-click *user-domain* CA trust is wired and shipping.** With SSL on and a host in scope, Loom MITM-decrypts and captures HTTPS. Apple domains legitimately fail (cert pinning) — expected, not a bug. A client only trusts the leaf if Loom's root CA is trusted. Three ways, in order of what's actually available:
  ...
  - **Privileged helper (system-domain) — dormant by decision, do not schedule work against it.** Loom signs ad-hoc only, so `PrivilegedHelperClient` + `LoomHelper` can never load: `SMAppService.register()` refuses a root daemon with no trust anchor, and `HelperIdentity.callerCodeRequirement` (`anchor apple generic and identifier "com.loom.app"`) refuses an ad-hoc caller over XPC even if it did. Both refusals are correct — a root process that installs system-wide CA trust must not accept a forgeable binary. The code (hardened design, caller + Apple-binary validation, per-service proxy backup/restore, crash watchdog, idle-exit; pure logic unit-tested) stays in the tree as a design record, but **nothing invokes it, CI never exercises it, and it is not a pending task**. User-domain trust above is the shipping path.
...
- **`MainView` uses `HSplitView` deliberately — do not swap it back to `NavigationSplitView`.** With live traffic + tail-follow, a sidebar category switch inside `NavigationSplitView` pays a quadratic AppKit KVO teardown across the row hosting views accumulated by scrolling: measured **8.7–16 s of main thread vs 143–224 ms** under `HSplitView` (61×) on the same view. Both containers land on `NSSplitView`; only `NavigationSplitView`'s private pane glue stalls, so the defect is in the container, not split views or anything Loom does per row (all per-row work sums to ~18 ms). Sidebar collapse is hand-rolled (toolbar `sidebar.left`, `.navigation` placement, ⌃⌘S). Full two-round investigation record (sample stacks, A/B matrix, lldb findings): [`docs/performance/navigation-split-view-kvo.md`](docs/performance/navigation-split-view-kvo.md); DESIGN.md § main-window records the container decision.
