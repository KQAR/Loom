...
**Next (ordered — see [`ROADMAP.md`](ROADMAP.md))**: finish M2 (privileged helper for system-trust install + system-proxy — an unverified scaffold, blocked on Developer ID signing/notarization); then the breakpoint supervision gap — `BreakpointControlling` is complete in the engine and reachable over MCP, but `ProxyClient`/`AppFeature` expose none of it, so an agent can hold real traffic with nothing in the human's panel showing it or releasing it.
