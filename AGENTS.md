...
**Next (ordered — see [`ROADMAP.md`](ROADMAP.md))**: M7 is nearly closed — breakpoint supervision shipped, and HAR import/export are now recorded as deliberately agent-only (they touch no live traffic, so their absence is not a supervision gap). What remains is internal: whether `ProxyClient` should wrap `any ProxyControlling`, and collapsing rule authoring's four representations. Neither is user-visible, so the next user-facing round should be chosen from real usage pain rather than from the remaining checklist. M2's privileged helper is **not** on this list: it is parked by owner decision (ad-hoc signing only — see Known Issues and ROADMAP § M2), not blocked. Breakpoint supervision is **closed**: `BreakpointsFeature` mirrors armed + held state (boot seed → `pendingBreakpointStream` → re-sync after every write, plus a slow poll only while something is held — a hold can also resolve with no decision, on client hangup or the watchdog), surfaced in the **sidebar → Breakpoints** panel and a conditional orange console row. Editing a held exchange stays agent-only (`resume` over MCP); the human's two decisions are proceed-unmodified and abort — deliberately no second write path onto the same continuation.
