...
**In scope now also (M8, done — capture reach)**: a **SOCKS5 listener** (`SOCKSServer` + `SOCKSConnectionHandler`, bound one port above the HTTP proxy and reported as `ProxyStatus.socksPort` / `get_proxy_status.socksPort`) for the clients the HTTP proxy port can never see: anything honouring only `ALL_PROXY`/a SOCKS field, and anything that isn't HTTP at all. SOCKS names a destination *before* the client sends a byte, so the capture strategy is decided by sniffing the first bytes (`ProtocolSniff`: TLS record → MITM if in SSL scope, HTTP request line → cleartext capture, anything else incl. h2c prior knowledge → byte-transparent relay), not from the port number — the whole point is the traffic on non-standard ports. **Sniffing carries a 150 ms deadline (`SOCKSConnectionHandler.sniffDeadline`) and it is not a nicety**: SSH / SMTP / IMAP / MySQL / PostgreSQL are *server-first* — the server sends a banner before the client says a word — so classifying purely on client bytes deadlocks them outright (the client waits for a banner Loom hasn't opened an upstream connection to fetch). Verified against a real SSH server; the original integration test missed it because its opaque payload was client-first. A client that does speak first is already sitting on the socket, so only server-first connections pay the deadline. Both entry points install the *same* stack through `MITMPipeline`, because the intercepted handler names are load-bearing for the WebSocket upgrade and two copies is how one entry point rots. What this does **not** fix: a process that ignores every proxy setting (that needs transparent/pf redirection, deliberately not planned — see ROADMAP). Plus **mutual TLS**: `ClientCertificate` (PKCS#12 + passphrase, scoped by host glob) held in `ClientCertificateConfig` (`client-certificates.json`, 0600, validated on the way in, one `NIOSSLContext` cached per identity and dropped on any mutation) and consulted by `NIOStreamingForwarder` per upstream host, so an origin that *demands* a client certificate can be captured at all — without one the handshake fails and there is no exchange to record. Exposed as `list_client_certificates` / `set_client_certificate` / `delete_client_certificate`, wired into `ProxyClient` (which credential Loom presents to a third party is the human's to see and revoke) and surfaced in the **status-bar console** as a collapsed **Client Certificates** row under HTTPS (`ClientCertificatesCard`: list + `.fileImporter` add + confirmed delete). It is a row in the config console, **not** a sidebar panel: Rules / Breakpoints / Audit exist for activity that needs supervising *while it happens*, and an identity is installed once and then sits for months. The row appears while SSL is on **or** whenever an identity exists — the second half is what stops an agent's install from being invisible when interception happens to be off — and its detail reads "N need attention" when any identity is expired or unreadable, because both fail a handshake exactly like a missing one. Two rules ride along: a **configured-but-unloadable** identity throws rather than connecting without it (the error must name the identity, not the origin), and the audit trail **redacts** `pkcs12_base64` / `passphrase` (`MCPToolExecutor.redactedArgumentNames`) — supervision records the action, not the key. A refused handshake is wrapped in `UpstreamTLSError`, which names the host and **which identity Loom presented, or that it presented none** — the string lands in `Flow.error`, so it reaches `get_recent_flows` / `get_flow_detail` / the Inspector / HAR with no new `Flow` field. Two constraints there: it must be wrapped in `StreamingResponseHandler`, because a handshake failure does **not** come back from `connect()` (the TCP connection succeeds; the handshake fails afterwards inside the pipeline), and it must state what Loom *did*, never what the server *wanted* — a client-cert requirement arrives as a TLS alert, and under TLS 1.3 a rejection can surface after the handshake looks complete, indistinguishable from an ordinary reset. Only NIOSSL's own errors are wrapped for that reason; `ForwarderError.connectionClosed` deliberately is not. Per-flow attribution as a real `Flow` field is **deferred** (see ROADMAP M8): it answers "which identity", while the pain is "why did this fail", and the failure usually happens when there is no identity to attribute.
