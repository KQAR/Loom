...
- **The scope is a whitelist, and the unread first run is never silent (0.0.27).** *(verified 0.0.27.)* `toggleSSLTapped` seeds nothing: `include` starts empty and nothing is decrypted until a host is named. The direction is **name it, then decrypt**, which puts the whole weight on the unread or refused origin being *visible* — an unread relay records **no flow at all**, which is byte-for-byte what an agent sees when the client never ran. Two rules hold everywhere and so stay here: **a refused handshake reaches three surfaces — console, agent, request table — and none of them is optional**, and **nothing is pre-excluded and nothing is auto-excluded**, because a guessed list is both incomplete and quietly wrong, and auto-excluding a failing host would make this default silent again. The engine's invariants (`TunneledHostLog`, `RelayedTunnelRegistry`, what `intercept_host` reports) are in [`Engine/ProxyCore/CLAUDE.md`](Engine/ProxyCore/CLAUDE.md#the-ssl-scope-and-the-capture-drop-stage); the console card, the SSL column's four states and the row menu's two gates are in [`Features/AppFeature/CLAUDE.md`](Features/AppFeature/CLAUDE.md#the-ssl-scopes-human-surfaces). Why a whitelist rather than the wide default it replaces, the surfaces that were built and deleted, and the measurements behind each: [`docs/decisions/ssl-scope-whitelist.md`](docs/decisions/ssl-scope-whitelist.md).
