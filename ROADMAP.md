...
2. **A release has shipped in which the legacy path recorded no traffic.** Not a guess: `MCPProtocol` decides the era per request already, so counting legacy dispatches is a small change to make before the decision, not after it.
