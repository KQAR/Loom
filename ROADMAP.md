...
- **A root helper ships (0.0.17), scoped to the system proxy + pf QUIC block** — the thing that made every enable cost an admin password, since `/dev/pf` is root-only even for admins. It exists because half of the old "ruled out" was wrong: this entry used to say an XPC helper *cannot be enabled at all* on an ad-hoc build, resting on `SMAppService.register()` declining a root daemon with no trust anchor (**measured false** — a probe registered, reached `requiresApproval`, was approved once in Login Items and ran as uid 0) and on the caller requirement declining an ad-hoc app (real, but `anchor apple generic` was **our own string**). `HelperRequirement` now derives the strongest caller check the build's signature supports: identifier + team OU with a real identity, identifier alone when ad-hoc.
...
- **The container itself was the worst offender.** A sidebar category switch inside
  ...
  reproduced it. Full record in [`docs/decisions/navigation-split-view-kvo.md`](docs/decisions/navigation-split-view-kvo.md).
