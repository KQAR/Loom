...
- **Logging & audit trail — next.** The MCP write surface (replay / rules / breakpoints / ssl-scope) has no logging today. Add a durable **audit log of write actions** — what the agent did, surfaced to the supervising human and via an MCP read tool — plus error-level logging on the fail-open paths (corrupt CA, dropped persistence write, rule-apply errors). `os.Logger` stays; a pluggable backend waits until a non-Apple embedder needs it.
