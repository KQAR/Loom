...
2. **A release has shipped in which no legacy *handshake* was recorded.** This is measured now rather than guessed: `MCPProtocol.decide` returns the era **and the branch that chose it**, and `MCPEraLog` counts the branches apart. Read it with `get_version.protocolTraffic` — `legacyEraRetirable` is the answer, `legacyClients` names who is still on the old revision (which is also how condition 1 is checked).
