...
| `get_recent_flows` | newest-first flow summaries (method, url, status, host, flags). `captureTruncated: true` means a body (or the WS frame log) is only a prefix of what flowed — treat it as partial evidence |
