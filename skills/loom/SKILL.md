...
| Tool | Purpose |
| --- | --- |
| `set_system_proxy` | route this Mac's HTTP/HTTPS traffic through Loom (`enabled: true`), or turn the system proxy **off** (`enabled: false`). Disabling never restores a previous owner's settings (owner decision: re-pointing the proxy at an app that may have exited would break every request on the machine) — if another app held it, the user re-enables that app themselves. Machine-wide, may prompt for an admin password, and also blocks QUIC (UDP 443) so browsers fall back to TCP where a proxy can see them. Turn it off when done (Loom also turns it off on quit). A phone/other device doesn't need this — point that device at the proxy instead |
