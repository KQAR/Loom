...
components:
  ...
  inspector-panel: # bottom of the split — Request | Response, referenced from Proxyman
    ...
    requestPane: "tab strip [Summary · Query(n, only if any) · GraphQL(only if any) · Raw · Headers(n) · Cookies(n, only if any) · Body · Diff(replays only)] + method badge + Replay button; a copyable URL bar below the tabs. Raw = request line + headers + body with a line-number gutter."
