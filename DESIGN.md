...
system-design:
  ...
  scope: "System control metrics revert with it (measured on 26.5: NSButton 37×24 → 47×32, NSTextField h 24 → 21, titlebar 32 → 28pt). Loom's OWN metrics do not move — sidebar 300 default, {spacing}, {rounded}, the 7pt capture dot are all unchanged, and no view code is tuned to either design."
...
metrics:
  ...
  sidebar-width: 300         # category sidebar DEFAULT, not fixed: draggable 180-520 on its trailing edge, persisted (com.loom.sidebar.width) — a host list is as wide as its hostnames, which design time can't know. Also the collapse travel
...
components:
  ...
  sidebar:         # left column — categories
    ...
    hosts-are-a-tree: "hosts fold under their REGISTRABLE DOMAIN (`HostGrouping`: last two labels, or three under co.uk-style ccTLDs; no public-suffix list, since a coherent-but-imperfect group is still a correct filter), Charles-style. The domain row selects everything under it (`FlowCategory.domain`, the host dimension), a one-host domain is drawn flat, and the fold is the SAME chevron-in-the-row as devices-are-a-tree — but domains arrive CLOSED and the fold is NOT persisted (owner decision): the tree exists to fold hundreds of hosts, and a fold opened mid-chase is this session's. The parent glyph is 'globe'; children keep their favicons."
    decrypted-marker: "a host the SSL scope decrypts carries 'lock.open.fill' .caption2 .secondary beside its name — the request table's own glyph for an exchange Loom read — and SORTS ABOVE the rest: pinned, then decrypted, then alphabetical. Never a tint: it is configuration, and the sidebar's one coloured glyph is the held-breakpoint alert. A group floats when any member is decrypted and its tooltip says how many."
  ...
  inspector-panel: # bottom of the split — Request | Response, referenced from Proxyman
    ...
    pane-actions: "Body, Headers and Cookies pin a search + copy cluster top-right; Enter / chevrons step hits as 1/N and scroll the current one into view; copy takes the pane and flips to a checkmark."
...
- **Sidebar** (`.listStyle(.sidebar)`): `All Flows` / `Requests` / `Connections` / `Errors` / `Rules` / `Audit` / `Breakpoints`, then
  ...
  (`{components.sidebar}.devices-are-a-tree`); so is Hosts, folded by registrable domain with decrypted hosts
  marked and sorted first (`{components.sidebar}.hosts-are-a-tree`, `.decrypted-marker`). Selection is a **set** — ⌘-click composes filters, OR within a
