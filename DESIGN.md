...
components:
  ...
  sidebar:         # left column — categories
    ...
    disclosure-chevron: "ONE chevron for the section headers and the device rows — `chevron.right`, .caption2 semibold, .tertiary, rotated 90° when open. They were written twice and drew differently (a heavier .secondary down-chevron over a lighter .tertiary right-chevron), which read as two kinds of control rather than one control at two levels of a tree. One glyph rotated, never two swapped."
    device-glyph: "by platform — 'iphone' / 'ipad' / 'desktopcomputer' for this Mac, and Loom's own {components.android-glyph} for Android. Drawing every non-Apple device as 'iphone' is the kind of small lie that makes a sidebar harder to scan, not easier."
