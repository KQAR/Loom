...
1. **Sidebar — categories.** `All Flows`, `Errors`, then `Rules` / `Audit` / `Breakpoints` (which replace the table with the matching supervision panel), then `Devices` / `Apps` / `Hosts` sections grouping traffic by who sent it and where it went. Each row carries a plain trailing count, never a badge pill ([`DESIGN.md`](DESIGN.md) `{components.sidebar-counts}`). Selection scopes the table.
