...
///
/// Separated from the handlers because the two answer different questions —
/// "what can an agent ask for" versus "what happens when it does" — and because
/// this is the surface an agent reads before it ever calls anything, so it is
/// worth being able to read it end to end. `MCPServerTests` pins the two
/// together: every advertised tool must have a handler, and every tool marked as
/// a write action must be in `writeTools`.
