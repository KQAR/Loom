...
**And the census that guards the render side now guards the schema side too (0.0.28).** `RenderParityTests` had ~40 model→render entries; the schema side had one, `set_rule`, so every other write tool whose arguments mirror a model kept the original hole. `SchemaCensus` + `InputSchemaCensusTests` close it for the other six. **Add an entry when a new tool's arguments mirror a `SharedModels` type**; a tool taking scalars of its own invention has no model to drift from, and inventing one would be the second copy this approach exists to avoid.
